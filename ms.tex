...
\title{Spending too much time at the Galactic bar: Chaotic stream-fanning and the Ophiuchus stream}
...
The stream was later detected in BHB stars by identifying members using line-of-sight velocities: Twenty Blue Horizontal Branch (BHB) stars in the vicinity of the stream are kinematically distinct from the background halo stars and have distances consistent with the previously detected segment from PS1.
Four of these BHB stars near the end of the stream have similarly distinct velocities but have much larger velocity and spatial dispersions compared to the PS1 stream segment.
...
\todo{Move a lot of this to appendix?}
...
We generate mock streams along each of the mean orbits from the orbit fitting procedure (defined above). We use the method outlined above (Section \ref{sec:mocks}) to generate the streams and set the free parameters as follows: (1) we evolve the progenitors for 6 Gyr along each orbit, (2) we release star particles every 0.5 Myr (uniformly in time) to densely sample the final density distribution, (3) we set the progenitor mass to $m=10^4~\msun$, and (4) we set the disruption time of the progenitor equal to the last time at which a pericenter and disk crossing coincide (in each case, this is at $t \approx -200~{\rm Myr}$). 

Figures~\ref{fig:mockstream0}--\ref{fig:mockstream1} show the final particle positions and line-of-sight velocities in heliocentric coordinates for these mock streams (grey points). Vertical, dashed lines show the approximate extent of the part of the stream visible in main-sequence stars (excluding the BHB stars from S15b). We have applied the same distance-modulus cut to the points in these figures as used to select the BHB stars identified as stream members by S15b (see Section~2 in S15b). There are a few interesting features to note: 
\begin{enumerate}
	\item Even in the static potential (Figure~\ref{fig:mockstream0}, leftmost column), there is an increased dispersion in sky position for the stream stars towards higher Galactic longitudes, $l$. This is a projection effect: The portion of the stream at larger $l$ is closer and almost pointing towards the Sun, as demonstrated in Figure~\ref{fig:mockstreamxyz}. Figure~\ref{fig:mockstreamxyz} shows $x$-$z$ projections of the star particles in Galactocentric Cartesian coordinates (grey) along with the position of the Sun (symbol at $(x,z)=(-8,0)~{\rm kpc}$) and the corners of the Heliocentric, sky-position plots of Figures~\ref{fig:mockstream0}--\ref{fig:mockstream1}.
	\item The density of the mock stream in the static potential decreases slowly rather than abruptly as is observed. This is more clearly seen in terms of the \todo{linear density of the stream??}, shown in Figure~\ref{fig:lineardensity}. .... \todo{Figure showing some brightness density vs longitude? Waiting on Branimir!}
...
To generate each mock stream, the star particles are released uniformly in time. The stream star particles at present day---e.g. the ``model'' points---are therefore a sampling of the stream model (phase-space) density field given a uniform stripping time distribution. We can then ask, given the data---a noisy sampling of the true density field---what stripping time distribution would reproduce the observed density field (e.g., the positions and velocities of the BHB stars). This corresponds to computing a weight for each $k$ model star particle with phase-space position $\bs{z}_k$ and release time, $t_{s,k}$:
\begin{align}
	w_k = \frac{1}{N} \, \sum^N_i \, p(\bs{z}_k \given \bs{x}_i, \bs{\sigma}_i, \bs{s})
\end{align}
(see Section~\ref{sec:orbitfit} for variable definitions). For the static potential all of the BHB stars associated with the stream must have been released very recently, so we expect that the inferred stripping time distribution for the mock stream in the static potential should be heavily skewed towards recent times. Indeed, the top left panel in Figure~\ref{fig:massloss} shows the stripping time distribution for the mock stream in the static potential: all stars would have to have been disrupted in the last $\lesssim 600~{\rm Myr}$ in order to produce the short segment of the stream observed. This is consistent with the findings and $N$-body simulations in S15a. 

\todo{This is wrong -- maybe axe? maybe bar3 is showing resonant stuff that the fardal, sanders paper talks about...}
For the mock streams in the barred potentials, the stripping time distributions are generally multi-modal and in some cases extend several Gigayears into the past, as we would expect for a disrupting globular cluster. In particular, the panel labeled bar3 shows a case in which there could be many ``mass-loss'' events over the history of the cluster's orbit, but the final debris distribution would still remain short. This is because the star particles released earlier will diffuse away and therefore won't get penalized by the weighting scheme described above.
...
If the Ophiuchus stream formed from a globular cluster on a chaotic orbit and we happen to be witnessing its final demise, what does this imply about the population of clusters that have already been fully disrupted? For a given model for the Galactic bar, we can study the orbit structure in the inner Milky Way and, in particular, study the fraction of strongly chaotic orbits that would lead to chaotic fanning or fast dispersal of tidal debris \citep{recent work by zotos or those greek peeps}. The existence of strongly chaotic orbits in this region would limit the expected number of cold stellar streams in the inner Galaxy. Comparing the observed numbers with expectations ... \todo{what to say exactly -- KVJ?}
...
\caption{{\it Left column:} Circular velocity curves along the Sun-Galactic center line for a representative barred MW potential model (top left) and for the static MW potential model (bottom left). Solid black line shows total (sum of all components), lines below show a decomposition by potential component. Vertical grey bar shows approximate position of the Sun, horizontal grey bar shows roughly the range in measured circular velocity of the Sun. {\it Right column:} Contours of constant surface density for a barred MW potential (top right) and the static MW potential model (bottom right). Four contours are drawn per decade in surface density between $10^7$ and $10^{12}~\msun~{\rm kpc}^{-3}$. Note the perturbation from the bar potential within Galactocentric radius $r \lesssim 4~{\rm kpc}$. The Sun's position is indicated by the `$\odot$' symbol. }
...
\begin{figure}[p]
\begin{center}
\includegraphics[width=\textwidth]{figures/surface-density}
\caption{ Surface density of mock star particles ... \todo{This is with the same distance cut as used for the BHB stars -- is that what we want to use?}}	
